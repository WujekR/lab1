...
enum class WeaponType { LASER, BULLET, SQUAREBOMB, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		...
		else if (type == WeaponType::SQUAREBOMB) {
			DrawRectanglePro({ transform.position.x, transform.position.y, 12.f, 12.f },
				{ 6.f, 6.f }, transform.rotation, ORANGE);
		}
	}
...
inline static Projectile MakeProjectile(WeaponType wt, const Vector2 pos, float speed)
{
	if (wt == WeaponType::LASER) {
		return Projectile(pos, { 0, -speed }, 20, wt);
	}
	else if (wt == WeaponType::SQUAREBOMB) {
		return Projectile(pos, { 0, -speed * 0.5f }, 5, wt);
	}
	else { // BULLET
		return Projectile(pos, { 0, -speed }, 10, wt);
	}
}
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						if (pit->GetType() == WeaponType::SQUAREBOMB) {
							Vector2 origin = pit->GetPosition();
							...
							for (int i = 0; i < 8; ++i) {
								...
								projectiles.emplace_back(origin, vel, 2, WeaponType::BULLET); 
							}
						}
